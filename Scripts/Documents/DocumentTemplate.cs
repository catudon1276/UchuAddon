...
static public class RoleDocumentHelper
{
    ...
    static public GUIWidget? GetAssignableNameWidget(DefinedAssignable? assignable)
    {
        ...
        return gui.VerticalHolder(GUIAlignment.TopLeft,gui.HorizontalHolder( GUIAlignment.Left,gui.VerticalHolder(GUIAlignment.Left, gui.VerticalMargin(0.05f), gui.RoleIcon( GUIAlignment.Center,assignable, 0f,new(0.55f, 0.55f), 0.05f)), gui.HorizontalMargin(0.02f),
            gui.VerticalHolder(GUIAlignment.Left, blurb,gui.VerticalMargin(-0.06f),title,gui.VerticalMargin(-0.05f)),gui.HorizontalMargin(0.25f),citationWidget ),
            GetRoleFilterContent(assignable),gui.VerticalMargin(0.15f),GetDocumentText(assignable?.ConfigurationHolder?.Detail.GetString()),gui.VerticalMargin(0.15f));
    }
    ...
    static public GUIWidget GetImageContent(string nebulaImagePath, GUIWidget widget) => GetImageContent(NebulaAPI.AddonAsset.GetResource(nebulaImagePath)?.AsImage(100f)!, widget);
    static public GUIWidget GetImageContent(string nebulaImagePath, string rawText) => GetImageContent(NebulaAPI.AddonAsset.GetResource(nebulaImagePath)?.AsImage(100f)!, GetDocumentText(rawText));
    static public GUIWidget GetImageLocalizedContent(string nebulaImagePath, string translationKey) => GetImageContent(NebulaAPI.AddonAsset.GetResource(nebulaImagePath)?.AsImage(100f)!, GetDocumentLocalizedText(translationKey));
    static public GUIWidget GetImageLocalizedContent(string nebulaImagePath, string translationKey, Func<string, string> decorator) => GetImageContent(NebulaAPI.AddonAsset.GetResource(nebulaImagePath)?.AsImage(100f)!, GetDocumentLocalizedText(translationKey, decorator));
